...
function mota_settings_fields_validate($inputs) // Validate : nettoie et adapte les valeurs de nos réglages (optionel mais permet des vérifications lors de l’enregistrement des données).
{
     if (!empty($_POST)) {
          ...
          if (!empty($_POST['mota_settings_field_phone_number'])) {
               update_option('mota_settings_field_phone_number', $_POST['mota_settings_field_phone_number']);
          }
          if (!empty($_POST['mota_settings_field_email'])) {
               update_option('mota_settings_field_email', $_POST['mota_settings_field_email']);
          }
     }
...
// Création de 'hook' d'action WP 
add_action('admin_menu', 'mota_add_admin_pages', 10); // 10 est un chiffre par défaut représentant la priorité de l'action.
